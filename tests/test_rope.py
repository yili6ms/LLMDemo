...
class TestApplyRope:
    ...
    def test_rope_identity_case(self):
        ...
        assert torch.allclose(result, x, atol=1e-6), "RoPE with cos=1, sin=0 should preserve input"
...
class TestRopeInAttention:
    ...
    def test_attention_rope_vs_no_rope(self):
        ...
        attn_rope = CausalSelfAttention(
            ...
            max_seq_len=max_seq_len, use_rope=True
        )
        
        attn_no_rope = CausalSelfAttention(
            d_model=d_model, n_heads=n_heads, dropout=0.0,
            max_seq_len=max_seq_len, use_rope=False
        )
        ...
        assert not torch.allclose(output_rope, output_no_rope, atol=1e-5), \
    ...
    def test_rope_cos_sin_initialization(self):
        ...
        assert torch.all(attn.cos >= -1) and torch.all(attn.cos <= 1), "cos values should be in [-1, 1]"
        assert torch.all(attn.sin >= -1) and torch.all(attn.sin <= 1), "sin values should be in [-1, 1]"
    ...
    def test_rope_frequency_calculation(self):
        ...
        attn = CausalSelfAttention(
            d_model=d_model, n_heads=n_heads, use_rope=True
        )
        
        # Check inverse frequency calculation
        expected_inv_freq = 1.0 / (10000 ** (torch.arange(0, head_dim, 2).float() / head_dim))
        
        assert torch.allclose(attn.inv_freq, expected_inv_freq, atol=1e-6), \
...
class TestRopeInModel:
    ...
    def test_model_rope_vs_learned_positions(self):
        ...
        assert model_rope.pos_emb is None, "RoPE model should not have positional embeddings"
        assert model_learned.pos_emb is not None, "Learned position model should have positional embeddings"
    ...
    def test_rope_model_generation(self):
        ...
        assert torch.all(output >= 0) and torch.all(output < 50), "Generated tokens should be valid"
    ...
    def test_rope_model_parameter_count(self):
        ...
        expected_diff = config['max_seq_len'] * config['d_model']  # Positional embedding size
        assert params_learned - params_rope == expected_diff, \
...
class TestRopeNumericalStability:
    ...
    def test_rope_large_values(self):
        ...
        # Large input values
        x = torch.tensor([[[[1000.0, -1000.0, 500.0, -500.0],
        ...
        assert torch.all(torch.isfinite(result)), "RoPE should handle large values without overflow"
    ...
    def test_rope_small_values(self):
        ...
        assert not torch.allclose(result, torch.zeros_like(result)), "Should not underflow to zero"
