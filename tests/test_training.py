...
from train import (
    get_batch, get_lr_scheduler, train_step, evaluate, 
...
class TestTrainingUtils:
    ...
    def test_get_batch(self):
        ...
        for i in range(batch_size):
            ...
            assert torch.equal(x[i], expected_x), f"Row {i}: expected {expected_x}, got {x[i]}"
            assert torch.equal(y[i], expected_y), f"Row {i}: expected {expected_y}, got {y[i]}"
    ...
    def test_lr_scheduler_warmup(self):
        ...
        for step in range(warmup_steps):
            ...
            assert abs(actual_lr - expected_lr) < 1e-6, f"Step {step}: expected {expected_lr}, got {actual_lr}"
            scheduler.step()
    ...
    def test_lr_scheduler_cosine_decay(self):
        ...
        warmup_steps = 10
        max_steps = 100
        scheduler = get_lr_scheduler(optimizer, warmup_steps, max_steps, min_lr_ratio=0.1)
        ...
        assert abs(final_lr - expected_min_lr) < 1e-6, f"Expected min LR {expected_min_lr}, got {final_lr}"
    ...
    def test_train_step(self):
        ...
        for initial_param, current_param in zip(initial_params, model.parameters()):
            assert not torch.equal(initial_param, current_param), "Parameters should change after training step"
    ...
    def test_evaluate(self):
        ...
        metrics = evaluate(model, val_data, batch_size=4, seq_len=10, device=device, num_batches=5)
    ...
    def test_save_load_checkpoint(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Save checkpoint
            save_checkpoint(model, optimizer, scheduler, step=123, val_loss=2.5, 
                          config=config, path=checkpoint_path)
            ...
            checkpoint_data = load_checkpoint(checkpoint_path, new_model, new_optimizer, new_scheduler)
...
class TestTrainingIntegration:
    ...
    @pytest.mark.parametrize("use_rope,use_flash", [
        (False, False),
        (False, True),
        (True, False),
        (True, True)
