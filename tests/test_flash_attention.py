...
class TestFlashAttentionFallback:
    ...
    def test_sdpa_detection_on_init(self):
        ...
        with patch('model.gpt.torch.nn.functional.scaled_dot_product_attention', side_effect=ImportError):
    ...
    def test_flash_vs_manual_attention_equivalence(self):
        ...
        with torch.no_grad():
            ...
            def mock_sdpa_implementation(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False):
            ...
            with patch('torch.nn.functional.scaled_dot_product_attention', side_effect=mock_sdpa_implementation):
            ...
            assert torch.allclose(output_flash, output_manual, atol=1e-5, rtol=1e-4), \
    ...
    def test_model_with_flash_attention(self):
        ...
        with patch('torch.nn.functional.scaled_dot_product_attention') as mock_sdpa:
            mock_sdpa.return_value = torch.randn(batch_size, 4, seq_len, 8)  # Mock return value
...
class TestFlashAttentionPerformance:
    ...
    def test_flash_attention_different_seq_lengths(self, seq_len):
        ...
        with patch('torch.nn.functional.scaled_dot_product_attention') as mock_sdpa:
            mock_sdpa.return_value = torch.randn(batch_size, n_heads, seq_len, d_model // n_heads)
