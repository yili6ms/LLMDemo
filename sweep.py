...
def run_experiment(config: dict[str, Any], experiment_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"experiment_id": experiment_id, "status": "timeout", **config}
...
def main():
    ...
    successful_results = [
        r for r in results if r["status"] == "completed" and r.get("final_loss")
    ]
    ...
    if successful_results:
        ...
        for i, result in enumerate(successful_results[:5]):
            ...
            print(
                f"   d_model={result['d_model']}, n_layers={result['n_layers']}, "
        ...
        with open(csv_path, "w") as f:
            f.write(
                "experiment_id,d_model,n_layers,learning_rate,dropout,final_loss,final_perplexity,status\n"
            )
            for result in results:
                f.write(
                    f"{result['experiment_id']},{result.get('d_model', '')},{result.get('n_layers', '')},"
