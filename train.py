...
def train(config: Dict[str, Any]) -> None:
    ...
    # Mixed precision training
    scaler = GradScaler() if device.type == 'cuda' and config.get('mixed_precision', True) else None
    
    # Resume from checkpoint
    start_step = 0
    best_val_loss = float('inf')
    if config.get('resume_from'):
        checkpoint = load_checkpoint(Path(config['resume_from']), model, optimizer, scheduler)
    ...
    for step in range(start_step, config['max_steps']):
        ...
        if step % config.get('eval_interval', 1000) == 0 and step > 0:
            val_metrics = evaluate(model, val_data, config['batch_size'], config['seq_len'], device)
            ...
            if val_metrics['loss'] < best_val_loss:
                ...
                save_checkpoint(
                    model, optimizer, scheduler, step, best_val_loss, config,
    ...
    # Final evaluation
    final_metrics = evaluate(model, val_data, config['batch_size'], config['seq_len'], device, num_batches=100)
