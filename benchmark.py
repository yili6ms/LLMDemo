...
def benchmark_forward_pass(model, batch_size=8, seq_len=128, num_iterations=100, device='cuda'):
...
def benchmark_training_step(model, batch_size=8, seq_len=128, num_iterations=50, device='cuda'):
...
def benchmark_generation(model, tokenizer, prompt="The quick brown fox", max_tokens=100, device='cuda'):
...
def run_comprehensive_benchmark():
    ...
    for config in configs:
        ...
        try:
            ...
            print(f"    Generation: {generation_results['tokens_per_second']:.1f} tok/s")
...
def benchmark_flash_attention():
    ...
    if 'manual' in results and 'flashattention' in results:
        if 'tokens_per_second' in results['flashattention'] and 'tokens_per_second' in results['manual']:
            speedup = results['flashattention']['tokens_per_second'] / results['manual']['tokens_per_second']
...
def main():
    ...
    with open(args.output, 'w') as f:
        ...
        f.write("| Config | Parameters | Forward (tok/s) | Training (tok/s) | Generation (tok/s) |\n")
        f.write("|--------|------------|-----------------|------------------|--------------------|\n")
        ...
        for config_result in benchmark_results['configs']:
            if 'error' not in config_result:
                ...
                f.write(f"| d{config['d_model']}_l{config['n_layers']} | {params:,} | {forward_tps:.0f} | {train_tps:.0f} | {gen_tps:.1f} |\n")
        ...
        if 'speedup' in flash_results:
            ...
            if 'manual' in flash_results:
                f.write(f"| Manual Attention | {flash_results['manual']['tokens_per_second']:.0f} |\n")
            if 'flashattention' in flash_results:
                f.write(f"| FlashAttention | {flash_results['flashattention']['tokens_per_second']:.0f} |\n")
        else:
